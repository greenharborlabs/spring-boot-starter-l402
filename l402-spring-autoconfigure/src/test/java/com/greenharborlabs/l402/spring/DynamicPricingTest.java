...
/**
 * T086: Verifies that dynamic pricing via {@link L402PricingStrategy} overrides
 * the static {@code priceSats} value from the {@link L402Protected} annotation.
 *
 * <p>Scenario: {@code @L402Protected(priceSats = 50, pricingStrategy = "myPricer")}
...
class DynamicPricingTest {
    ...
    @Test
    @DisplayName("invoice uses dynamic price (150) from pricing strategy, not static annotation value (50)")
    void invoiceUsesDynamicPriceFromStrategy() throws Exception {
        mockMvc.perform(get(DYNAMIC_PATH))
    ...
    static class DynamicPriceController {
        ...
        @L402Protected(priceSats = 50, pricingStrategy = "myPricer")
    ...
    /**
     * Extended stub that captures the amountSats parameter passed to createInvoice,
     * allowing tests to verify the correct price was requested.
     */
