...
/**
 * Backward compatibility test (T040): verifies that when only existing
 * {@code paygate.*} properties are set — with NO MPP configuration — the
 * system operates in L402-only mode, producing the same response format
 * as before the dual-protocol refactoring.
 *
 * <p>Uses real auto-configuration ({@link PaygateAutoConfiguration} and
 * {@link TestModeAutoConfiguration}) to prove that the default config
...
class BackwardCompatibilityTest {
    ...
    @Test
    @DisplayName("402 response body has code, message, price_sats, description, invoice, and protocols fields")
    ...
    @Test
    @DisplayName("402 response body has protocols object (L402Protocol provides null bodyData, so protocols is empty)")
    ...
    void l402EndToEndAuthFlow() throws Exception {
        ...
        var challengeResult = mockMvc.perform(get(PROTECTED_PATH))
                .andExpect(status().isPaymentRequired())
                .andReturn();
