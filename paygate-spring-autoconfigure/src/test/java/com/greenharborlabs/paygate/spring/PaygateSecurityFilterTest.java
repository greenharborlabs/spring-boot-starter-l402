 ...
 * avoiding dependency on PaygateAutoConfiguration which does not yet exist.
 */
...
class PaygateSecurityFilterTest {
    ...
    @Autowired
    private PaygateEarningsTracker earningsTracker;

    @Autowired
    private PaygateSecurityFilter paygateSecurityFilter;
    ...
    static class TestApp {
        ...
        PaygateEndpointRegistry paygateEndpointRegistry() {
            ...
            registry.register(
                    new PaygateEndpointConfig("GET", CAPABILITY_PROTECTED_PATH, PRICE_SATS, 600, "Capability protected endpoint", "", "search")
            );
            return registry;
        }
